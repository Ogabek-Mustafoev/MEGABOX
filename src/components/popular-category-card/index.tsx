...
export const PopularCategoryCard: FC<IPopularCategoryCard> = ({ text = 'Televizorlar', icon }) => {
  return (
    <div className="flex w-full min-w-60 bg-light-gray gap-3 p-3 h-full items-start justify-between rounded-3xl overflow-hidden cursor-pointer">
      <p className="line-clamp-2 mt-2 leading-5 font-semibold text-black">{text}</p>
      <div className="flex items-center justify-center  rounded-xl">
        <Image
          className="rounded-none w-36 h-24 object-contain"
          src={icon}
